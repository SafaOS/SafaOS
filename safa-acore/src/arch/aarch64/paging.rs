...
use crate::misc::{Frame, FrameIter, IterPage, Page, PhysAddr, VirtAddr};
...
impl Entry {
    ...
    fn frame(&self) -> Option<Frame> {
        ...
        {
            return Some(Frame::containing(
                // TODO: simplify this
                // 47 bits set after the first 12 bits
                PhysAddr::new((self.0 & (((1 << 47) - 1) << 12)) as usize),
            ));
        }
        None
    }
    ...
    const fn new(flags: ArchEntryFlags, addr: PhysAddr) -> Self {
        Self(addr.raw() as u64 | flags.bits())
    }
    ...
    fn mapped_to(&self) -> Option<&'static mut ArchPageTable> {
        if let Some(frame) = self.frame() {
            let phys_addr = frame.addr();
            let virt_addr = *crate::bootloader::HHDM + *phys_addr;
            let entry_ptr = virt_addr.into_ptr::<ArchPageTable>();
    ...
    unsafe fn free(&mut self, level: u8) {
        unsafe {
            ...
            if level != 0 {
                let table = &mut *((*HHDM + *frame.addr()).into_ptr::<ArchPageTable>());
                table.free(level);
            }
            self.deallocate();
        }
...
pub unsafe fn current_kernel() -> NonNull<ArchPageTable> {
    let ttbr1_el1: usize;
    unsafe {
        asm!("mrs {}, ttbr1_el1", out(reg) ttbr1_el1);
        let addr = PhysAddr::from(ttbr1_el1);
        NonNull::new_unchecked((*HHDM + *addr).into_ptr::<ArchPageTable>())
    }
}

/// Returns the current lower half root table
pub unsafe fn current_user() -> NonNull<ArchPageTable> {
    let ttbr0_el1: usize;
    unsafe {
        ...
        NonNull::new((*HHDM + *addr).into_ptr::<ArchPageTable>()).expect("user page table is null")
    }
}
...
impl PageTableOps for ArchPageTable {
    ...
    fn finish_ops(&mut self, pages: crate::misc::IterPage) {
