...
struct VMMInner {
    start_addr: VirtAddr,
    size: usize,
    root_objects_set: NonNull<VMMObjectsPage>,
...
impl VMMInner {
        ...
        while let Some(curr_obj) = current {
            ...
            if curr_obj.addr() <= start_addr && curr_obj.region_end() >= end_addr {
                ...
                let (new_head, new_tail) = curr_obj
                    ...
                    .map_err(|PMMError::OutOfMemory| VMMAllocError::OutOfMemory)?;
    ...
    fn debug_regions(&self) {
        logging::debug!(VirtualMemoryManager, "Memory Regions: ");
    ...
    fn deallocate_at(&mut self, addr: VirtAddr) -> Option<(ObjectState, usize)> {
        ...
        while let Some(obj) = current.take() {
            ...
            assert!(
                ...
                "Attempt to free memory inside of the object's range and not at the start, addr: {:?}, obj: {:?}-{:?}, {:?}",
                addr,
                obj.addr(),
                obj.region_end(),
                { self.debug_regions() }
...
impl Drop for VMMInner {
    fn drop(&mut self) {
        ...
        while let Some(mut page) = curr {
            let r = unsafe { page.as_mut() };
            let next = r.next.take();
            // TODO: unmap memory on drop?
            unsafe {
                pmm::deallocate_frame(VMMObjectsPage::frame(page))
                    .expect("Failed to unmap VMM metadata")
            };
            curr = next;
        }
    }
...
impl VirtualMemoryManager {
    ...
    pub fn new(start_addr: VirtAddr, size: usize, page_table: PageTable) -> Self {
        let mut objects =
        ...
        let object_ptr = unsafe { objects.as_mut() }
            .add_object(object)
            .expect("Failed to insert object");

        VirtualMemoryManager {
            inner: IntSpinLock::new(VMMInner {
                start_addr,
                size,
        ...
        for (obj_addr, deallocated, del_size) in deallocs {
            match deallocated {
                ...
                ObjectState::DMAAllocated(_) => {
                    unsafe { op.unmap_no_dealloc(obj_addr, del_size.div_ceil(PAGE_SIZE), false).expect("Failed to unmap VMM Allocated memory") };
                },
            }
        }
    ...
    pub fn unmap(&self, start_addr: VirtAddr) -> bool {
        ...
        match deallocated {
            ...
            ObjectState::DMAAllocated(_) => {
                let mut op = self.page_table.lock();
                drop(inner);
                unsafe { op.unmap_no_dealloc(start_addr, del_size.div_ceil(PAGE_SIZE), false).expect("Failed to unmap VMM Allocated memory") };
            },
        }
...
//             )
//             .map_err(|e| {
//                 logging::error!(VirtualMemoryManager, "VMM map returned error: {e:?}");
...
//         let new_size = new_layout.size().next_multiple_of(PAGE_SIZE);
//         let old_size = old_layout.size().next_multiple_of(PAGE_SIZE);
...
//                 Err(VMMAllocError::OutOfMemory) => {
//                     // TODO: Handle OOM
//                     logging::error!(VirtualMemoryManager, "OOM!!!!");
