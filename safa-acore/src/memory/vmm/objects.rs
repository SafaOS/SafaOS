...
use crate::memory::pmm::PMMError;
use crate::memory::{phys_to_virt, virt_to_phys};
use crate::memory::{pmm, vmm::VMMMFlags};
use crate::misc::{Frame, PAGE_SIZE, VirtAddr};
...
const EXTRA_PAGE_BYTES: usize = size_of::<u128>() /* bitmap */ + size_of::<Option<NonNull<VMMObjectsPage>>>() /* next */;
...
pub struct VMMObjectsPage {
    ...
    pub next: Option<NonNull<VMMObjectsPage>>,
}
...
impl VMMObjectsPage {
    ...
    #[inline(always)]
    pub unsafe fn frame(this: NonNull<Self>) -> Frame {
        Frame::containing(virt_to_phys(VirtAddr::from_ptr(this.as_ptr())))
    }

    pub fn allocate() -> Result<NonNull<Self>, PMMError> {
        pmm::allocate_frame().map(|frame| {
            let ptr = phys_to_virt(frame.addr()).into_ptr::<Self>();
            unsafe { *ptr = Self::new() };
    ...
    pub fn add_object(&mut self, obj: VMMObject) -> Result<NonNull<VMMObject>, PMMError> {
        ...
        if index >= self.objects.len() {
            ...
            return if let Some(ref mut next) = self.next {
                unsafe { next.as_mut().add_object(obj) }
            } else {
                let next = Self::allocate()?;
                unsafe { self.next.insert(next).as_mut().add_object(obj) }
            };
        }
...
impl VMMObject {
    ...
    pub fn split_at(
        ...
        offset: usize,
        size: usize,
    ) -> Result<(Option<NonNull<Self>>, Option<NonNull<Self>>), PMMError> {
        if offset == 0 {
    ...
    pub fn split_to_fit(&mut self, fit: usize) -> Result<Option<NonNull<Self>>, PMMError> {
